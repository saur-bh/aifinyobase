...
class FinetradingPage extends BasePage {
  ...
  verifypurchasing() {
    ...
    cy.wait(1000);
    cy.get(locator.purchaceboxvalue)
      .should('be.visible')
      .and('not.have.text', ' ');
      ...
      cy.get('@firstText').then((firstText) => {
        cy.get('@secondText').then((secondText) => {
          cy.get('@lastText').then((lastText) => {
            ...
            var freepurchase = parseFloat(
              firstText.split(' ')[0].replace(',', ''),
            );
            var usedpurchase = parseFloat(
              secondText.split(' ')[0].replace(',', ''),
            );
            var totalpurchase = parseFloat(
              lastText.split(' ')[0].replace(',', ''),
            );
  ...
  verifyToastMessage(text) {
    cy.get(locator.errortoast, { timeout: 10000 })
  ...
  verifyInvoiceNumberClickTable() {
    cy.get(locator.tablerow)
      .should('be.visible')
      .first()
      .find(locator.datarownumber)
      .invoke('text')
      .as('invoicenumber');
    //
